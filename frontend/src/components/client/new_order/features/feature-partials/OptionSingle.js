...
import { newOrderToggleOptionSingle } from '../../../../../actions/newOrder';
import * as currency from "currency.js"
import {currencyOptions} from "../../../../../helpers/regional"
class OptionMany extends React.Component {
    render =()=> <div className={`special-field ${this.props.field.type}`}>
    ...
    <div className={`special-field-options`}>
        {this.props.field.options.map(option =>
            <div
                key={option.id}
                onClick={() => this.props.newOrderToggleOptionSingle(this.props.field.id, option.id)}
                className={`special-field-option ${option.value ? 'active' : ''}`}>
                <div className={`radio-button`}>
                    <div className={`radio-button-dott`} />
                </div>
